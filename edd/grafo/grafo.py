...
class Vertice:
    def __init__(self, _id: str) -> None:
        ...
        self._entrantes: set[Vertice] = set()  # Nuevo conjunto para los nodos entrantes
    ...
    @property #Propiedad para acceder a entrantes
    def entrantes(self) -> set[Vertice]:
        return self._entrantes
...
class Grafo:
    ...
    def agregar_arista(self, origen: str, destino: str, peso: int | float | None = None) -> None:
        ...
        # Actualizar el conjunto de nodos entrantes
        vertice_destino._entrantes.add(vertice_origen)

        if not self._dirigido:
            self.__agregar_arista(vertice_destino, vertice_origen, peso)
            vertice_origen._entrantes.add(vertice_destino)  # Actualiza también para grafos no dirigidos
